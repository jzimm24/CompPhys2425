...
def nearestNeighbourJulius(points, start, distances):
    N = len(points)
    path = np.zeros(N+1)
    path[0] = start
    mask = np.zeros(N)
    ...
    for i in range(N-1):
        currentPoint = int(path[i])
        mask[currentPoint-1] = 1
        relaventDist = ma.masked_array(distances[currentPoint-1][:], mask)
        nextPoint = np.argmin(relaventDist)+1
        path[i+1] = nextPoint
...
def doubleSidedNearestNeighbourJulius(points, start, distances): ###Check if this works i for odd N
    N = len(points)
    pathLeft = np.zeros(int(N/2))
    pathRight = np.zeros(int(N/2))
    pathLeft[0] = start
    pathRight[0] = start
    mask = np.zeros(N)
    ...
    for i in range(int(N/2-1)):
        #print(mask)
        #print(i)
        currentPointLeft = int(pathLeft[i])
        currentPointRight = int(pathRight[i])
        #print(currentPointLeft, currentPointRight)
        mask[currentPointLeft-1] = 1
        relaventDistLeft = ma.masked_array(distances[currentPointLeft-1][:], mask)
        nextPointLeft = np.argmin(relaventDistLeft)+1
        pathLeft[i+1] = nextPointLeft
        mask[nextPointLeft-1] = 1
        mask[currentPointRight-1] = 1
        relavantDistRight = ma.masked_array(distances[currentPointRight-1][:], mask)
        nextPointRight = np.argmin(relavantDistRight)+1
        pathRight[i+1] = nextPointRight
        mask[nextPointRight-1] = 1

    if (N%2 == 0):
        
        lastPoint = np.argmin(ma.masked_array(distances[0][:], mask))+1
        pathLeft = np.append(pathLeft, lastPoint)
        

    path = np.append(pathLeft, np.flip(pathRight))    

    return(path)

def nearestNeighborAllJulius(points, distances):
    """
        Calculate way with minial distance through Nearest_Neighbor algorithm
        Input: points - points of interests - integer array from 0-#cities-1
               distances - 2d-matrix containing the distances
        Output: way with minimal_distances- by Nearest_Neighbor algorithm
                minimal_distance
                all paths (different startning points)
                all weights
    
    """
    paths = np.zeros((len(points), len(points)+1))
    weights = np.zeros(len(points))

    for k in range(len(points)):
        paths[k][:] = nearestNeighbourJulius(points, points[k], distances)
        weights[k] = base.totalTravelDist(paths[k][:].astype(int)-1, distances)


    min_weight = np.min(weights)
    best_path = paths[np.argmin(weights)][:]

    return(best_path, min_weight, paths, weights)

def doubleSidedNearestNeighborAllJulius(points, distances):
    """
        Calculate way with minial distance through Nearest_Neighbor algorithm
        Input: points - points of interests - integer array from 0-#cities-1
               distances - 2d-matrix containing the distances
    ...
    for k in range(len(points)):
        paths[k][:] = doubleSidedNearestNeighbourJulius(points, points[k], distances)
        weights[k] = base.totalTravelDist(paths[k][:].astype(int)-1, distances)
...
def christofidesAllJulius(A, points):
    """
    Input: distances_ Matrix conatining distances between points
            points - points tf
    Output: Best solution of tsp with Christofides algorithm over all starting points
    """    
    weights = np.zeros((len(points)))
    paths = np.zeros((len(points)+1, len(points)+1))
    for s in range(len(A)):
            paths[s, :] = tsp.christofides(A,s)
            weights[s] = base.totalTravelDist(paths[s, :].astype(int),A)
