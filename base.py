...
def traveling_MC_constDist_adapativeTemp(points, dist_matrix, tries, alpha = 0.999, method =2):
    """ input:  points - points that should be visited by each path (in some order) - array
                dist_matrix - matrix containing the 'distances' between each point - 2darray
                tries - itterations of the Monte Carlos - Markov chain - int
                starting_temp - scalar in the exponent of the markov chain acception process - int
    ...
    temps = np.zeros(tries+1)
    pathWeights[0] = totalTravelDist(points, dist_matrix)
    temps[0] = np.max(dist_matrix)*100
    cost = pathWeights[0]
...
def traveling_MC_reuse_constDist_adapativeTemp(points, dist_matrix, tries, resue=5, alpha = 0.999, method =2):
    """ input:  points - points that should be visited by each path (in some order) - array
                dist_matrix - matrix containing the 'distances' between each point - 2darray
                tries - itterations of the Monte Carlos - Markov chain - int
                starting_temp - scalar in the exponent of the markov chain acception process - int
        ...
        return: points - points that should be visited by each path (in some order) - array
                ...
                temps - temperatures after each MC step - array of int
    """
    pathWeights = np.zeros((tries+1)*reuse)
    temps = np.zeros((tries+1)*reuse)
    
    for i in range(reuse):
        traveling_MC_constDist
...
def traveling_MC_constDist_history_adaptiveTemp(points, dist_matrix, tries, alpha = 0.999, method=2):
    """ input:  points - points that should be visited by each path (in some order) - array
                dist_matrix - matrix containing the 'distances' between each point - 2darray
                tries - itterations of the Monte Carlos - Markov chain - int
                starting_temp - scalar in the exponent of the markov chain acception process - int
...
def traveling_MC_reuse_constDist_history_adaptiveTemp(points, dist_matrix, reuse = 5, tries = 10000, temp_reuse = 500, alpha = 0.999, method=2):

    pathHistoryComplete = np.zeros((reuse*(tries+1), len(points)+1))
    pathWeightsComplete = np.zeros(reuse*(tries+1))
    tempsComplete = np.zeros(reuse*(tries+1))
    temp = np.max(dist_matrix)*100

    for i in range(reuse):
        pathWeights, pathHistory, temps = traveling_MC_constDist_history(points.astype(int), dist_matrix, tries, temp, alpha, method)
        for k in range(tries+1):
            pathHistoryComplete[i*(tries+1)+k, :] = pathHistory[k]
            pathWeightsComplete[i*(tries+1)+k] = pathWeights[k]
            tempsComplete[i*(tries+1)+k] = temps[k]
        pointsLong = pathHistoryComplete[np.argmin(pathWeightsComplete[:(i*tries+1)]), :]
        points = np.delete(pointsLong,-1)
        #temp = temp_reuse

    return(pathWeightsComplete, pathHistoryComplete, tempsComplete)

def traveling_MC_reuse_constDist_history_adaptiveTemp_noPaths(points, dist_matrix, reuse = 5, tries = 10000, temp_reuse = 500, alpha = 0.99, method=2):
    ...
    for i in range(reuse):
        ...
        for k in range(tries+1):
            pathWeightsComplete[i*(tries+1)+k] = pathWeights[k]
            tempsComplete[i*(tries+1)+k] = temps[k]
        pointsLong = pathHistory[np.argmin(pathWeights), :]
        points = np.delete(pointsLong,-1)
        #temp = temp_reuse

    return(pathWeightsComplete, tempsComplete)
